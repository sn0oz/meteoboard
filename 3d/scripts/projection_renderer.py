...
class Renderer(object):
    def __init__(self, input_file, output_folder, extra_variables=None):
        ...
        try:
            self.mark_enabled = self.extra_variables['render_mark']
        except KeyError:
            self.mark_enabled = True
        try:
            self.engrave_enabled = self.extra_variables['render_engrave']
        except KeyError:
            self.engrave_enabled = True
    ...
    def _render_component(self, i, panel_horizontal, panel_vertical):
        ...
        if self.mark_enabled:
            style_options.append('mark')
        if self.engrave_enabled:
            style_options.append('engrave')
        for style in (style_options):
            ...
            try:
                _ = openscad.run(
                        ...
                        variables=self._get_variables({
                            ...
                            'render_mark': style == 'mark',
                            'render_engrave': style == 'engrave',
            ...
            if style == 'cut':
                ...
                processor.apply_laser_mark_style()
            elif style == 'engrave':
                processor.apply_laser_engrave_style()
            return processor
